...
ES & YM - Market Profile, VWAP, Bias-Ampel
Setup:
  python -m pip install finnhub-python yfinance numpy
  set FINNHUB_API_KEY=dein_key
...
def main():
    ...
    es = process_instrument("ES=F", "E-Mini S&P 500")
    ym = process_instrument("YM=F", "E-Mini Dow Jones")
    ...
    print("\n>>> Bias berechnen...")
    es["bias"] = compute_bias(es.get("market_profile"), es.get("current_price"), ctx)
    ym["bias"] = compute_bias(ym.get("market_profile"), ym.get("current_price"), ctx)
